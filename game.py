        ...
        for evento in pygame.event.get():
            ...
            if evento.type == pygame.KEYDOWN:
                ...
                if evento.key == pygame.K_a:
                    ...
                    with open('pygamesc/dataset_guardados/datos_modelo.csv', 'w') as f:
                        for dato in datos_modelo:
                            f.write(f"{dato[0]},{dato[1]},{dato[2]}\n")
                    print("Datos guardados en datos_modelo.csv")
                    pygame.quit()
                    exit()
...
def entrenar_modelo():
    ...
    if len(datos_modelo) < 30: # Se asegura de que hay suficientes datos para entrenar, 30 es un número arbitrario
        print("No hay suficientes datos para entrenar.")
        return

    print("Entrenando modelo con", len(datos_modelo), "datos...")

    X = np.array([[v, d] for v, d, s in datos_modelo])
    Y = np.array([s for _, _, s in datos_modelo])

    # Se define un modelo de red neuronal con 2 capas ocultas de 5 neuronas cada una y un maximo de 1000 iteraciones
        ...
        for evento in pygame.event.get():
            ...
            if evento.type == pygame.KEYDOWN:
                ...
                if evento.key == pygame.K_q:  # Presiona 'q' para terminar el juego
                    print("Juego terminado. Datos recopilados:", datos_modelo)
                    # Crear un archivo csv para guardar los datos recopilados o actualizar si ya existe
                    with open('pygamesc/dataset_guardados/datos_modelo.csv', 'w') as f:
                        for dato in datos_modelo:
                            f.write(f"{dato[0]},{dato[1]},{dato[2]}\n")
                    print("Datos guardados en datos_modelo.csv")
